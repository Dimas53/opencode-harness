...
reads the index, and later sessions treat what they find there as how this
project works. So when an agent that had just been blocked from
`git commit --no-verify` wrote under **Workarounds** that "the correct move is
to hand the user the exact commands and let them run it in their terminal", it
did not record a mistake — it filed one as practice, three hours after the
rules had been changed to forbid exactly that.

Hard Limits now end with the rule that was missing: a blocked limit is not a
workaround to remember, and if it is worth a note at all, the note is that the
limit held. `session-end` says the same when it prompts for the day's memory
file.

**Also fixed, from the same run:** the generated index carried the marker
"generated by `scripts/index-memory.sh`" — a path relative to the harness,
sitting in a client project where that file does not exist. The agent ran it,
got "no such file", and hand-edited the generated block, which the marker
exists to prevent. It now names `session-end`, which works from inside any
adopted project, and the block is matched by prefix so changing the wording
cannot silently produce a second index below the first.
...
yesterday's fix. `is_ours()` looked for `Installed by: scripts/install-hooks.sh`.
That is the header of the old **pre-commit** hook. The old **post-commit** hook,
written by the same repo, says `Installed by: scripts/install.sh,
scripts/update.sh`. So pre-commit was recognised and replaced in place, while
post-commit was filed away as a stranger's backup — one hook handled correctly,
the other producing exactly the debris the fix exists to prevent.

The confusing part on the surface: `post-commit.bak` carries an mtime of
2026-08-05, because `mv` preserves it. It looks like an old file; it was created
today.

Now matched by prefix — `Installed by: scripts/` — plus a harness marker
(`dod.sh`, `OPENCODE_HARNESS_PATH`, or `config/opencode/skills`, the last one
because post-commit's job is mirroring). Test added for the second header
specifically.

Both `.bak` files in `itocook` were verified to be harness hooks (not the
user's) and moved out of `.git/hooks/` into the scratchpad, reversibly.

**Worth recording separately: no checker could have caught this.** Both hooks
are generated by this repo, both are covered by tests — but the tests used the
one header the fix was written against. It took running the thing on a real
project, which is the argument the live-validation file (notes 16) makes in its
opening paragraph, now with one more example.
